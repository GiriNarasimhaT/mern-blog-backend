...
app.get('/posts', async (req,res)=>{
    res.json(await Post.find().select('-content -viewCount -likeCount -comments')
        .populate('author',['_id','username']).sort({createdAt:-1}).limit(20));
});
...
//post view count
app.put('/post/:id/viewcount', async (req, res) => {
    try {
      const { id } = req.params;
      const postDoc = await Post.findById(id);
      if (!postDoc) {
      ...
      await postDoc.updateOne({ $inc: { viewcount: 1 } });
...
//post like count
app.put('/post/:id/likecount', async (req, res) => {
    try {
      const { id } = req.params;
      const postDoc = await Post.findById(id);
      if (!postDoc) {
      ...
      await postDoc.updateOne({ $inc: { likecount: 1 } });
...
//post unlike count
app.put('/post/:id/unlikecount', async (req, res) => {
    try {
      const { id } = req.params;
      const postDoc = await Post.findById(id);
      if (!postDoc) {
      ...
      await postDoc.updateOne({ $inc: { likecount: -1 } });
      res.json({ message: 'Like count decremented' });
    } catch (error) {
      res.status(400).json({ error: 'Invalid id' });
    }
});
...
app.post('/post/:id/comments', async (req, res) => {
  try {
      ...
      if (!postDoc) return res.status(404).send('Post not found');
      
      postDoc.comments.push({ text: comment });
      await postDoc.save();
